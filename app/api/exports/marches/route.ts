...
import { exportMarches } from '@/lib/actions/exports'

// Force dynamic pour éviter le cache
export const dynamic = 'force-dynamic'

/**
 * API Route - Export des marchés en Excel
 * GET /api/exports/marches?statut=...&type=...&dateDebut=...&dateFin=...
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await exportMarches(filters)
    ...
    console.error('[API_EXPORT_MARCHES]', error)
    return NextResponse.json(
      { error: "Erreur lors de l'export des marchés" },
      { status: 500 }
    )
  }
}
