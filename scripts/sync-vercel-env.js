...
// ============================================
//
// SÉCURITÉ : ce fichier ne doit plus jamais contenir de valeurs en dur.
// Les valeurs sont lues depuis .env.local (non commité, voir .gitignore) —
// jamais depuis ce fichier source, qui est versionné et donc public.
// Incident du 2026-02-09 : SMTP_PASS et CRON_SECRET réels étaient codés en
// dur ici et exposés publiquement sur GitHub jusqu'au 2026-07-09.
...
const SENSITIVE_KEYS = new Set(['SMTP_PASS', 'CRON_SECRET']);
const DEFAULT_TARGETS = ['production', 'preview', 'development'];

function buildVariablesFromEnv() {
  const missing = REQUIRED_ENV_KEYS.filter((key) => !process.env[key]);
  if (missing.length > 0) {
    throw new Error(
      `Variables manquantes dans .env.local : ${missing.join(', ')}\n` +
      `Renseignez-les localement (fichier jamais commité) avant de relancer ce script.`
    );
  }
...
const CONFIG = {
  ...
  variables: buildVariablesFromEnv(),
};
